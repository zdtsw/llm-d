...
- NVIDIA GPU Operator and NFD Operator - The original installation instructions can be found in the [NVIDIA GPU Operator on OpenShift documentation](https://docs.nvidia.com/datacenter/cloud-native/openshift/latest/steps-overview.html). However below are convenient steps for setting up via configuration given by RedHat AI Business Unit.
