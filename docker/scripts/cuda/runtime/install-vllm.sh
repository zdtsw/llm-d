...
# Add NIXL if it wasn't built from source in the dockerfile builder stage (build-nix.sh)
...
if [ -z "${WHEEL_INDEX_HTML}" ]; then
  ...
  MACHINE=$(uname -m)
  case "${MACHINE}" in
    x86_64|amd64) PLATFORM_TAG="manylinux1_x86_64" ;;
    aarch64|arm64) PLATFORM_TAG="manylinux2014_aarch64" ;;
...
# We built our own NVSHMEM in the dockerfile builder stage, so we don't need the one from vLLM as depedency
if [[ "${NVSHMEM_DIR-}" != "" ]]; then
